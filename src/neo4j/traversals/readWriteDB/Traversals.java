...
//cc added at 20160319
	public static String getFunctionNamefromId(long nodeId)
	{
		
		System.out.println("Get into the function of  getFunctionNamefromId!");
		//Neo4JDBInterface.openDatabase();
		//System.out.println("openDatabase successfully!");
		//Neo4JDBInterface neointer = new Neo4JDBInterface();
		/*if(Neo4JDBInterface == null)
		{
			System.out.println("Get into the function of  getFunctionNamefromId!");
			return "NULL";
		}
		...
		if(node.getProperty(NodeKeys.TYPE).toString()=="Function")
		{
			System.out.println(node.getProperty(NodeKeys.NAME).toString());
			return node.getProperty(NodeKeys.NAME).toString();
			}
		else if(node.getProperty(NodeKeys.FUNCTION_ID).toString() == null)
		{
			System.out.println("This is not in Function!");
			return "This is not in Function!";
			}
		else
		{
			long functionId=new Long(node.getProperty(NodeKeys.FUNCTION_ID).toString());
			Node fun_node = Neo4JDBInterface.getNodeById(functionId);
			System.out.println(fun_node.getProperty(NodeKeys.NAME).toString());
			return fun_node.getProperty(NodeKeys.NAME).toString();
			}
		}
}